...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        ...
        if node.op is None:
            continue
        adjoint = node.op.gradient_as_tuple(node_grad, node)
        for (input_node,partial_adjoint) in zip(node.inputs,adjoint):
            if input_node not in node_to_output_grads_list:
                node_to_output_grads_list[input_node] = []
            node_to_output_grads_list[input_node].append(partial_adjoint)
    # for node in reverse_topo_order:
    #     try:
    #         print(node.grad,node.requires_grad)
    #     except Exception:
    #         print('--------------------------------------')
    #         print(type(node))
    ### BEGIN YOUR SOLUTION
...
def topo_sort_dfs(node, visited, topo_order):
    ...
    # 列表在python中是可变类型，通过函数内部成员变量进行修改时对形参的改变会改变实参的值
    visited.append(node)
